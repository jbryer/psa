...
ggplot(dat, aes(x = ps, y = outcome)) +
	geom_point(aes(size = ate_weight, color = as.logical(treatment)), alpha = 0.5) +
	geom_hline(yintercept = 0) + 
	geom_smooth(method = 'loess', se = TRUE, formula = y ~ x, aes(weight = ate_weight)) +
	# geom_smooth(method = 'lm', se = TRUE, formula = y ~ x, aes(weight = ate_weight)) +
