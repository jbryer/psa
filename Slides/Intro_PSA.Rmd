...
```{r, eval=FALSE, echo=FALSE, fig.align='right', out.width='20%'}
...
knitr::include_graphics('Causality.png')
```
...
<center><img src='Causality2.png' width='500' alt="Causality - Missing Values" /></center>
...
ggplot() +
	...
	geom_segment(data = strata5_out, aes(x = xmin, xend = xmax, y = mean, yend = mean, color = treatment), size = 2) +
	geom_point(data = dat, aes(x = ps, y = outcome, color = as.logical(treatment)), alpha = 0.5) +
	geom_text(data = breaks5$labels, aes(x = xmid, y = -2.5, label = strata), color = 'black', vjust = 0.7, size = 5) +
	scale_color_manual('Treatment', values = palette2) +
	xlab('Propensity Score') +
	ggtitle('Scatter plot of propensity scores versus outcome',
			subtitle = 'Five strata represented by vertical lines')
```
...
ggplot() +
	geom_point(data = dat, aes(x = ps, y = outcome, color = as.logical(treatment)), alpha = 0.5) +
	geom_segment(data = dat_match, aes(x = treat_ps, xend = control_ps,
...
ggplot(dat, aes(x = ps, y = outcome, color = as.logical(treatment))) +
	...
	scale_color_manual('Treatment', values = palette2) +
	ggtitle('Scatter plot of propensity scores versus outcome',
			subtitle = 'Point sizes correspond to propensity score weights')
...
---
class: inverse, middle, center
# Example: National Supported Work
...
loess_plot(psadf[psadf$Y < 30000,]$ps, response=psadf[psadf$Y < 30000,]$Y, 
...
[`r icons::fontawesome("link")` github.com/jbryer/psa](https://github.com/jbryer/psa)   
